...
bool SparseFeatConfig::initParams(unordered_map<string, shared_ptr<ParamContainerInterface>> & shared_param_container_map) {
  ...
  if (!embedding_existed) {
    param_container = std::make_shared<ParamContainer<FtrlParamUnit>>(vocab_size, mutex_nums);
...
int SparseFeatContext::feedSample(const char *feat_str, size_t feat_str_len, FmLayerNode & fm_node) {
  ...
  feat_param = cfg_.param_container->get(feat_id);
  param_mutex = cfg_.param_container->GetMutexByFeatID(feat_id);
...
void SparseFeatContext::backWard(FMParamUnit &backward_grad) {
